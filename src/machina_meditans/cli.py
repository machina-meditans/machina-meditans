...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Machina-Meditans thinks for you about the meanings.",
        epilog="Example:  machina-meditans input_text.txt > output_text.txt"
...
def run():
    """
    $ text | machina-meditans                        # Accepts text from the pipe
    $ echo "...<text>..." | machina-meditans         #

    $ machina-meditans multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="machina-meditans",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"machina-meditans: {utterance}.")
        syslog.closelog()
